...
app.get('/api/users/:eventId', (req, res) => {
  ...
  const real = Object.values(users).filter(u =>
    ...
    .map(u => ({ username: u.username, lat: u.lat, lng: u.lng, role: u.role, heat: HEAT_VALUES[u.role] || 1, blurb: u.blurb || '', offering: u.offering || '', car: u.car || '', instagram: u.instagram || '', car_photo: u.car_photo || '', email: u.email || '', locationEnabled: u.locationEnabled !== false }));
  res.json([...real, ...demo]);
});
