...
app.post('/api/dummy-users', (req, res) => {
  var { username, role, lat, lng, event_id, blurb, offering, locationEnabled, car, instagram, car_photo, email, display, photo } = req.body;
  if (!username) return res.status(400).json({ error: 'username required' });
  demoUsers.push({ username, role: role || 'attendee', lat: lat || 40.5144, lng: lng || -111.4764, event_id: event_id || 'demo-event-1', blurb: blurb || '', offering: offering || '', locationEnabled: locationEnabled !== false, car: car || '', instagram: instagram || '', car_photo: car_photo || '', email: email || '', display: display || '', photo: photo || '' });
  res.json(demoUsers);
});
...
app.get('/api/users/:eventId', (req, res) => {
  ...
  const real = Object.values(users).filter(u =>
    ...
    .map(u => ({ username: u.username, lat: u.lat, lng: u.lng, role: u.role, heat: HEAT_VALUES[u.role] || 1, blurb: u.blurb || '', offering: u.offering || '', car: u.car || '', instagram: u.instagram || '', car_photo: u.car_photo || '', email: u.email || '', photo: u.photo || '', locationEnabled: u.locationEnabled !== false }));
  res.json([...real, ...demo]);
});
